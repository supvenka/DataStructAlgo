...
public class Queue {
	...
	public void enQueue(int data)
	{
		if(isFull())
		{
			...
			queue[rear] = data;
			rear ++; //incrementing rear since we will insert next element at index rear+1 so queue[rear+1] basically rear will be pointer to where we will add the next item 
			size++;
		}
	...
	public int denqueue()
	{
		if(isEmpty())
		{
			...
			System.arraycopy(queue, front+1, newQueue, 0, newQueSize); //copy from queue  index [1] since index[0] is removed till index [length -1]
	...
	public int size()
	{
		return rear;	// we can also do return size. 
